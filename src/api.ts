...
export const DESKA_NEXT_API_URL = process.env.DESKA_NEXT_API_URL;
console.log('DESKA_NEXT_API_URL', DESKA_NEXT_API_URL);

export const createKnowledgeBaseCollection = async ({
  name,
  description,
}: {
  name: string;
  ...
  console.log('createKnowledgeBaseCollection');
  const token = getJwtToken();
  console.log('token', token);
  return request
    .post(`${DESKA_NEXT_API_URL}/api/collections`)
    .set('Authorization', token)
    .send({
      name,
      description,
      // TODO: remove hardcoded account_id
      account_id: 'd0338f46-661a-4ef0-b56d-5656e77dfa15',
    })
    .then(
      (res) =>
        res.body.data as {
...
export const fetchKnowledgeBaseCollections = async () => {
  const token = getJwtToken();
  return request
    .get(`${DESKA_NEXT_API_URL}/api/collections`)
    .set('Authorization', token)
    .then(
      (res) =>
        res.body as {
          data: Array<KnowledgeBaseCollection>;
          status: 'ok' | any;
        }
    );
};

export const deleteKnowledgeBaseCollection = async (id: string) => {
  const token = getJwtToken();
  return request
    .delete(`${DESKA_NEXT_API_URL}/api/collections?collectionId=${id}`)
    .set('Authorization', token)
    .then((res) => res.body);
};

export const knowledgeBaseApi = {
  fetchKnowledgeBaseCollections,
  createKnowledgeBaseCollection,
  deleteKnowledgeBaseCollection,
};

export const knowledgeBaseArticlesApi = {
  fetchDocuments: ({collectionId}: {collectionId: string}) => {
    const token = getJwtToken();
    return request
      .get(
        `${DESKA_NEXT_API_URL}/api/collections/documents?${new URLSearchParams({
          collectionId,
        })}`
      )
      .set('Authorization', token)
      .then(
        (res) =>
          res.body as {
            data: Array<KnowledgeBaseArticle>;
            status: 'ok' | any;
          }
      );
  },
...
export const preprocessDocument = async ({
  ...
  const token = getJwtToken();
  return request
    .post(`${DESKA_NEXT_API_URL}/api/documents-loader/web-page`)
    .set('Authorization', token)
    .send({urls, collectionId})
    .then(
      (res) =>
        res.body.data as {
          url: string;
...
export const uploadKnowledgeBaseDocuments = async ({
  documents,
  collectionId,
  accountId,
}: {
  ...
  const token = getJwtToken();
  return request
    .post(`${DESKA_NEXT_API_URL}/api/collections/upload`)
    .set('Authorization', token)
    .send({documents, collectionId, accountId})
    .then((res) => res.body);
};

export const fetchUrlSitemaps = async ({url}: {url: string}) => {
  const token = getJwtToken();

  return request
    .post(`${DESKA_NEXT_API_URL}/api/collections/explore-url`)
    .set('Authorization', token)
    .send({urls: [url]})
    .then(
      (res) =>
