...
export class NotificationsProvider extends React.Component<Props, State> {
  ...
  connect = (channel: string) => {
    ...
    this.channel.onError(() => {
      logger.error(
        `Error connecting to notification channel. Attempting reconnect after ${RETRY_AFTER}ms...`
      );

      this.timeout = setTimeout(() => this.connect(channel), RETRY_AFTER);
    });
    ...
    this.channel
      ...
      .receive('error', (err) => {
        ...
        logger.info(`Attempting reconnect after ${RETRY_AFTER}ms...`);
      ...
      .receive('timeout', (data) => {
        ...
        logger.info(`Attempting reconnect after ${RETRY_AFTER}ms...`);
        // TODO: double check that this works (retries after RETRY_AFTER ms)
        this.timeout = setTimeout(() => this.connect(channel), RETRY_AFTER);
      });
  };
