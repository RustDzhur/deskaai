    ...
    <Box>
      <DetailsSectionCard>
        ...
        <Flex mb={1} sx={{alignItems: 'center'}}>
          <MailOutlined style={{color: colors.primaryAccent}} />
          <Box
            ml={2}
            sx={{
              maxWidth: '100%',
        ...
        <Flex mb={1} sx={{alignItems: 'center'}}>
          <PhoneOutlined style={{color: colors.primaryAccent}} />
          <Box ml={2}>{phone || 'Unknown'}</Box>
        </Flex>
        <Flex mb={1} sx={{alignItems: 'center'}}>
          <UserOutlined style={{color: colors.primaryAccent}} />
          <Box
            ml={2}
            sx={{
              maxWidth: '100%',
      ...
      <DetailsSectionCard>
        ...
        <Box mb={1}>
          <span>IP:</span> {lastIpAddress || 'Unknown'}
        </Box>
      </DetailsSectionCard>
      ...
      {/* <DetailsSectionCard>
        <Box mb={2}>
          <Link to={`/customers/${customer.id}?tab=issues`}>
            <span className="font-bold">Linked issues</span>
          </Link>
  ...
  return notification.open({
    ...
    description: (
      <Box>
        ...
        <Box
          mb={3}
          className=" border-border border-[1px] bg-background rounded-md"
        >
...
const ConversationDetails = ({conversation}: {conversation: Conversation}) => {
  ...
  return (
    <Box px={2} py={3} sx={{borderTop: '1px solid rgba(0,0,0,.06)'}}>
      ...
      <Box px={2} mb={1}>
        <span>Status:</span>{' '}
        <Badge variant={status === 'open' ? 'default' : 'danger'}>
          {status}
        </Badge>
      </Box>
      <Box px={2} mb={3}>
        <span>Source:</span> <Badge variant={'secondary'}>{source}</Badge>
      </Box>
...
const ConversationDetailsSidebar = ({
  ...
  return (
    <Box
      id="conversation-details-sidebar"
      sx={{
        width: '100%',
        minHeight: '100%',
        // bg: 'rgb(250, 250, 250)',
