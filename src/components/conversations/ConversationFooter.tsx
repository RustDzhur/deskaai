...
import {
  ...
  MenubarMenu,
  MenubarTrigger,
} from '../common/deska-kit-base/menu-bar';
...
const ConversationFooter = ({
  ...
  React.useEffect(() => {
    ...
    let isMounted = true; // New line to track if the component is mounted

    Promise.all([API.fetchAccountUsers(), API.fetchCannedResponses()]).then(
      ([users, responses]) => {
        // Before setting state, ensure component is still mounted
        if (isMounted) {
    ...
    return () => {
      isMounted = false; // On unmount, change the isMounted variable to false
    };
  }, [conversationId]);
  ...
  return (
    <Box id="conversation-footer-input-box" style={{flex: '0 0 auto'}}>
      <Box
        sx={{
          bg: colors.bg,
          px: 4,
      ...
      >
        <Box
          ...
          pt={1}
          sx={{
            background: isPrivateNote ? colors.noteSecondary : colors.bg,
            ...
            <Box px={2} mb={2} sx={{position: 'relative'}}>
              ...
              <Menu
                ...
                style={{
                  ...
                  color:
                    messageType === 'note' ? colors.slate700 : colors.slate300,
                }}
            ...
            <Box mb={2}>
              ...
              <Mentions
                ...
                className={cn(
                  ...
                  messageType === 'note' ? 'ant-mentions-note' : ''
                )}
                placeholder={
                  isPrivateNote
