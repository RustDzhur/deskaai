...
import {Button, Paragraph, Text} from '../common';
import {Alert} from '../common/deska-kit-base/alert';
...
class LiveSessionViewer extends React.Component<Props, State> {
  ...
  renderStatusAlert() {
    ...
    if (loading) {
      return (
        <Alert
          message={<>Loading the current session... this may take a minute.</>}
          variant="info"
        />
      );
    } else if (!isCustomerConnected) {
      return (
        <Alert message={<>The customer has disconnected.</>} variant="danger" />
      );
      ...
      return (
        <Alert
          message={<>The customer is currently active.</>}
          variant="success"
        />
      );
    }
