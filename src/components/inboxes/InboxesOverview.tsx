...
import {colors, Container, Paragraph, Table, Tag, Text, Title} from '../common';
...
const InboxesTable = ({
        ...
        if (isPrimary) {
          return (
            <Flex sx={{alignItems: 'center'}}>
              ...
              <Box ml={3}>
                <Tag color={colors.primaryAccent}>Primary</Tag>
              </Box>
            </Flex>
          );
        } else {
  ...
  return (
    <LegacyDefaultTable loading={loading} dataSource={data} columns={columns} />
  );
};
...
class InboxesOverview extends React.Component<Props, State> {
  ...
  handleNewInboxCreated = (inbox: Inbox) => {
    ...
    this.props.history.push(`/inboxes/${inboxId}/chat-widget`);
  };
