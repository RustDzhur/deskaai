...
class IntegrationsOverview extends React.Component<Props, State> {
  ...
  fetchChatIntegration = async (inbox: Inbox): Promise<IntegrationType> => {
    ...
    const description = 'Chat with users on your website via Deska.';
  ...
  fetchSlackSupportIntegration = async (
    ...
    const description =
      auth && auth.channel && auth.team_name
        ? `Connected to ${auth.channel} in ${auth.team_name}.`
        : 'Sync messages from your Slack channels with Deska.';
  ...
  fetchGmailIntegration = async (inbox: Inbox): Promise<IntegrationType> => {
    ...
    return {
      ...
      authorizationId: auth ? auth.id : null,
      icon: '/gmail.svg',
      description: 'Sync messages from your Gmail inbox with Deska.',
  ...
  fetchEmailForwardingIntegration = async (
    ...
    return {
      ...
      icon: '/ses.svg',
      isPopular: true,
      description: 'Set up email forwarding into Deska.',
    ...
    return (
      <Container sx={{maxWidth: 960}}>
        <Box mb={5}>
          ...
          <Paragraph>
            <Text>
              Connect Deska with your favorite apps{' '}
          ...
          {inbox && inbox.id && (
            <Box my={4}>
              <Card
                sx={{p: 3, backgroundColor: colors.bg}}
