...
const EmptyElement: FC<{onClick: () => void; type?: 'upload' | 'sync'}> = ({
  onClick,
  type = 'upload',
}) => {
  return (
    <div className={s.emptyWrapper}>
      <div className={s.emptyElement}>
        ...
        <div className={s.emptyTip}>{``}</div>
        {type === 'upload' && (
          <Button size="xs" className=" mt-2" onClick={onClick}>
            <TbPlus className={s.plusIcon} />
            {'Add File'}
          </Button>
        )}
      </div>
...
const KnowledgeBaseDocuments: FC<IDocumentsProps> = ({
  collectionId,
}: {
  collectionId: string;
}) => {
  ...
  const documentsRes = useKnowledgeBaseStore(
    (state) => state.collectionDocuments?.[collectionId]
  );

  const {fetchKnowledgeBaseDocuments} = useKnowledgeBaseApi();

  useEffect(() => {
    fetchKnowledgeBaseDocuments(collectionId);
  }, []);

  const total = documentsRes?.length || 0;

  const routeToDocCreate = () => {
    history.push(`/knowledge-base/${collectionId}/upload`);
  };

  // TODO: FIXME: what is the equivalent of prefetch in react-router-dom?
  // router.prefetch(`/datasets/${datasetId}/documents/create`)

  const isLoading = !documentsRes;
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="flex flex-col justify-center gap-1 px-6 pt-4">
        <h1 className={s.title}>{'Documents'}</h1>
        <p className={s.desc}>{'All files of the dataset are shown her'}</p>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {documentsRes?.length ? (
            <Button
              ...
              // className="!h-8 !text-[13px]"
            >
              <TbPlus className="h-4 w-4 mr-2 stroke-current" />
              {'Add file'}
            </Button>
          ) : null}
        </div>
