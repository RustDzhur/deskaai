...
`;
const radioClass = `
  w-4 h-4 border-[2px] border-border rounded-full
`;
...
const IndexMethodRadio = ({value, onChange}: IIndexMethodRadioProps) => {
  const options = [
    {
      key: 'high_quality',
      text: 'datasetSettings.form.indexMethodHighQuality',
      desc: 'datasetSettings.form.indexMethodHighQualityTip',
      icon: 'high-quality',
    },
    {
      key: 'economy',
      text: 'datasetSettings.form.indexMethodEconomy',
      desc: 'datasetSettings.form.indexMethodEconomyTip',
      icon: 'economy',
    },
  ];
    ...
    <div className={classNames(s.wrapper, 'flex justify-between w-full')}>
      {options.map((option) => (
        ...
        >
          ...
          <div className="pl-9 text-xs text-gray-500 leading-[18px]">
            {option.desc}
          </div>
        </div>
      ))}
