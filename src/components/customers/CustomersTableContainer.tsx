...
import {Checkbox} from '../common/deska-kit-base/checkbox';
...
class CustomersTableContainer extends React.Component<Props, State> {
  ...
  handleToggleIncludeAnonymous = (e: CheckedState) => {
    this.setState({shouldIncludeAnonymous: e.valueOf() as boolean}, () =>
        ...
        {actions && typeof actions === 'function' && (
          <Flex mb={3} sx={{justifyContent: 'space-between'}}>
            ...
            <Flex mx={-2} sx={{alignItems: 'center'}}>
              {includeSearchInput && (
                <Box mx={2} className=" rounded-md " sx={{}}>
                  <Input.Search
                    className="_antd-search-table-btn"
              ...
              {includeAnonymousUserFilter && (
                <Checkbox
                  ...
                  onCheckedChange={this.handleToggleIncludeAnonymous}
                >
                  Include anonymous
                </Checkbox>
              )}
