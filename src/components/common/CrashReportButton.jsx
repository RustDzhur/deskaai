...
const MeteorCallShim = (s, s1, f) => {
  console.log(s, s1, f);
  f?.();
};
const wrapMeteorCallbackShim = (f) => {
  console.log(f);
  f?.();
};
...
const URL_WITH_BODY_TEMPLATE =
  'https://github.com/botfront/botfront/issues/new?labels=bug&template=bug_report.md&body=' +
  `
**Describe the bug**
Here is the report:
...
const CrashReportButton = (props) => {
  const {error, pathname: path, setReported, reported} = props;
  ...
  const generateReport = (text = true) => {
    ...
    const firstTwoLinesOfStack = err.stack
      ? err.stack.split('\n').slice(0, 2).join('\n    in ')
      : 'stack not available';
    if (text) {
      return (
    ...
    return {
      ...
      trace: `${firstTwoLinesOfStack}${info.componentStack || ''}`
        .replace(/\n\s{3,4}in\s/g, ' >> ')
        .split(' >> '),
    };
  };
  ...
  useEffect(
    () =>
      MeteorCallShim('reportCrash', generateReport(false), (_, res) => {
  ...
  return (
    <Button.Group fluid basic>
      <Button content="Report issue" onClick={sendCrashReport} />
      <Button content="Copy to clipboard" onClick={copyToClipboard} />
    </Button.Group>
  );
};
