...
const ICON_MAP = {
  ...
  api: (
    <AppIcon
      innerIcon={AlgorithmSvg}
      className="border !bg-purple-50 !border-purple-200"
    />
  ),
  dataset: (
    <AppIcon
      innerIcon={DatasetSvg}
      className="!border-[0.5px] !border-border !bg-indigo-25"
    />
  ),
};
...
export default function AppBasic({
  ...
  return (
    <div className="flex items-start">
      ...
      {iconType !== 'app' && (
        <div className="flex-shrink-0 mr-3">{ICON_MAP[iconType]}</div>
      )}
      <div className="group">
        <div
          className={`flex flex-row items-center text-sm font-semibold text-slate-200
           group-hover:text-slate-100 ${textStyle?.main}`}
        >
          {name}
          {hoverTip && (
