    ...
    >
      <div className={classNames('relative')}>
        ...
        {filteredItems.length > 0 && (
          <Combobox.Options className="absolute z-10 mt-1 px-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg border-border border-[0.5px] focus:outline-none sm:text-sm">
...
const SimpleSelect: FC<ISelectProps> = ({
  ...
  const localPlaceholder = placeholder || 'common.placeholder.select';
    ...
    >
      <div className={`relative h-9 ${wrapperClassName}`}>
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <Listbox.Options className="absolute z-10 mt-1 px-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg border-border border-[0.5px] focus:outline-none sm:text-sm">
            {items.map((item: Item) => (
              <Listbox.Option
                key={item.value}
                className={({active}) =>
