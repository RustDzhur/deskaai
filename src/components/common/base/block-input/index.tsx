...
const BlockInput: FC<IBlockInputProps> = ({
  ...
  const style = classNames({
    'block px-4 py-1 w-full h-full text-sm text-gray-900 outline-0 border-0 break-all':
      true,
        ...
        <div className="h-full px-4 py-1">
          <textarea
            ...
            className={classNames(
              editAreaClassName,
              'block w-full h-full absolut3e resize-none'
            )}
  ...
  return (
    <div
      className={classNames(
        'block-input w-full overflow-y-auto border-none rounded-lg'
      )}
    >
      {textAreaContent}
      {/* footer */}
        ...
        {isContentChanged ? (
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex space-x-2">
              <Button onClick={handleCancel} className="w-20 !h-8 !text-[13px]">
                {'Cancel'}
              </Button>
              <Button
          ...
          <p className="leading-5 text-xs text-gray-500">{'promptTip'}</p>
        )}
      </div>
    </div>
  );
