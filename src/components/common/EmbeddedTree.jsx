...
export default class EmbeddedTree extends Tree {
  ...
  componentWillUnmount = () =>
    this.treeContainer.removeEventListener('scroll', this.handleScroll);
  ...
  correctOffset = (deltas) => {
    ...
    const draggables = [
      ...this.treeContainer.querySelectorAll(
        '[data-react-beautiful-dnd-draggable]'
      ),
    ];
    const draggedItem = draggables.find(
      (elem) => elem.style.position === 'fixed'
    );
    ...
    if (offsetTop)
      draggedItem.style.top = `${
        parseInt(draggedItem.style.top, 10) - offsetTop
      }px`;
    if (offsetLeft)
      draggedItem.style.left = `${
        parseInt(draggedItem.style.left, 10) - offsetLeft
      }px`;
  };
