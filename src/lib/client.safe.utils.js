...
export const formNameIsValid = (name) =>
  name.match(/^[a-zA-Z0-9-_]+_form$/) && name.split('form').length === 2;

export const dropNullValuesFromObject = (obj) =>
...
export const insertSmartPayloads = ({
  rules = [],
  triggerIntent,
  ...fragment
}) => {
...
export const parseTextEntities = (text = '') => {
  ...
  const replaceEntities = (matchedText, relativeStart) => {
    ...
    let newText = `${matchedText.slice(1, end)}${matchedText.slice(
      pickupAfter
    )}`;
